...
sd.background_color = (127, 0, 127)
sd.resolution = (1200, 900)
sd.caption = 'Наше рабочее пространство'
first_point = sd.get_point(250, 250)
...
def benzol(start_point, length=200, angle=90):
    ...
    circle = sd.circle(center_position=sd.get_point(x_circle, y_circle), radius=length * 0.68, color=sd.COLOR_YELLOW, width=5)

    for number in range(6):
        vector = sd.get_vector(start_point=start_point, width=5, length=length, angle=angle + 60 * number)
        vector.draw()
...
def grid(resolution=sd.resolution, step=50):
    ...
    y_grid = step
    while x_grid < x:
        x_line = sd.line(start_point=sd.get_point(x_grid, 0), end_point=sd.get_point(x_grid, y), color=sd.COLOR_WHITE, width=1)
        x_grid += step

    while y_grid < y:
        y_line = sd.line(start_point=sd.get_point(0, y_grid), end_point=sd.get_point(x, y_grid), color=sd.COLOR_WHITE, width=1)
        y_grid += step
